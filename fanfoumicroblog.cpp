...
#include "fanfoueditaccount.h"
#include "fanfoupostwidget.h"
#include "fanfoutimelinewidget.h"
...
static const char apiUrl[] = "http://api.fanfou.com";

K_PLUGIN_FACTORY( FanfouMicroBlogFactory, registerPlugin<FanfouMicroBlog>(); )
K_EXPORT_PLUGIN( FanfouMicroBlogFactory( "choqok_fanfou" ) )

FanfouMicroBlog::FanfouMicroBlog( QObject* parent, const QVariantList& args )
: MicroBlog(FanfouMicroBlogFactory::componentData(), parent)
{
    Q_UNUSED(args)
    setServiceName( "Fanfou" );
    setServiceHomepageUrl( "http://fanfou.com/" );
    setCharLimit( 140 );
...
FanfouMicroBlog::~FanfouMicroBlog()
{
}

void FanfouMicroBlog::aboutToUnload()
{
    m_countOfTimelinesToSave = 0;
...
ChoqokEditAccountWidget* FanfouMicroBlog::createEditAccountWidget( Choqok::Account* account, QWidget* parent )
{
    FanfouAccount* acc = dynamic_cast<FanfouAccount*>(account);
    return new FanfouEditAccountWidget( this, acc, parent );
}

Choqok::UI::PostWidget* FanfouMicroBlog::createPostWidget( Choqok::Account* account, const Choqok::Post& post, QWidget* parent )
{
    FanfouAccount* acc = dynamic_cast<FanfouAccount*>(account);
    return new FanfouPostWidget( acc, post, parent );
}

Choqok::UI::TimelineWidget* FanfouMicroBlog::createTimelineWidget( Choqok::Account* account, const QString& timelineName, QWidget* parent )
{
    return new FanfouTimelineWidget( account, timelineName, parent );
}

void FanfouMicroBlog::createPost( Choqok::Account* theAccount, Choqok::Post* post )
{
    ...
    FanfouAccount* acc = dynamic_cast<FanfouAccount*>(theAccount);
...
void FanfouMicroBlog::abortCreatePost( Choqok::Account* theAccount, Choqok::Post* post )
{
...
void FanfouMicroBlog::fetchPost( Choqok::Account* theAccount, Choqok::Post* post )
{
    ...
    FanfouAccount* acc = dynamic_cast<FanfouAccount*>(theAccount);
    KUrl url( apiUrl );
    url.addPath( QString( "/statuses/show.json?id=%1" ).arg( post->postId ) );
...
void FanfouMicroBlog::removePost( Choqok::Account* theAccount, Choqok::Post* post )
{
    if ( !post || post->postId.isEmpty() ) {
...
void FanfouMicroBlog::saveTimeline( Choqok::Account* account, const QString& timelineName,
...
QList<Choqok::Post*> FanfouMicroBlog::loadTimeline( Choqok::Account* theAccount, const QString& timelineName )
{
    QList<Choqok::Post*> list;

    FanfouAccount* account = dynamic_cast<FanfouAccount*>(theAccount);
...
Choqok::Account* FanfouMicroBlog::createNewAccount( const QString& alias )
{
    FanfouAccount* acc = dynamic_cast<FanfouAccount*>(Choqok::AccountManager::self()->findAccount( alias ));
    if ( !acc )
        return new FanfouAccount( this, alias );
    else
        return 0;
}

void FanfouMicroBlog::updateTimelines( Choqok::Account* theAccount )
{
    FanfouAccount* acc = dynamic_cast<FanfouAccount*>(theAccount);
    if ( !acc )
        return;
...
Choqok::TimelineInfo* FanfouMicroBlog::timelineInfo( const QString& timelineName )
{
...
QString FanfouMicroBlog::postUrl( Choqok::Account* account, const QString& username, const QString& postId ) const
{
    Q_UNUSED(account)
    Q_UNUSED(username)
    return QString( "http://fanfou.com/status/%2" ).arg( postId );
}

QString FanfouMicroBlog::profileUrl( Choqok::Account* account, const QString& username ) const
{
    Q_UNUSED(account)
    return QString( "http://fanfou.com/%1" ).arg( username );
}

void FanfouMicroBlog::retweetPost( Choqok::Account* theAccount, Choqok::Post* post )
{
    if ( !post || post->postId.isEmpty() ) {
...
void FanfouMicroBlog::createFavorite( Choqok::Account *theAccount, Choqok::Post *post )
{
    if ( !post || post->postId.isEmpty() ) {
...
void FanfouMicroBlog::removeFavorite( Choqok::Account* theAccount, Choqok::Post* post )
{
    if ( !post || post->postId.isEmpty() ) {
        qWarning() << "Creating favorite failed. ID is empty.";
        return;
...
void FanfouMicroBlog::createFriendship( Choqok::Account* theAccount, Choqok::User* user )
{
...
void FanfouMicroBlog::removeFriendship( Choqok::Account* theAccount, Choqok::User* user )
{
    if ( !user || ( user->userId.isEmpty() && user->userName.isEmpty() ) ) {
...
void FanfouMicroBlog::slotCreatePost( KJob* job )
{
    if ( job->error() ) {
    ...
    FanfouAccount* acc = m_jobAccount.take( job );
...
void FanfouMicroBlog::slotFetchPost( KJob* job )
{
    if ( job->error() ) {
        qWarning() << "Job Error: " << job->errorString();
        return;
...
void FanfouMicroBlog::slotRemovePost( KJob* job )
{
    if ( job->error() ) {
        qWarning() << "Job Error: " << job->errorString();
        return;
...
void FanfouMicroBlog::slotRequestTimeline( KJob* job )
{
    if ( job->error() ) {
        qWarning() << "Job Error: " << job->errorString();
        return;
...
void FanfouMicroBlog::slotCreateFavorite( KJob* job )
{
    if ( job->error() ) {
        qWarning() << "Job Error: " << job->errorString();
        return;
...
void FanfouMicroBlog::slotRemoveFavorite( KJob* job )
{
    if ( job->error() ) {
        qWarning() << "Job Error: " << job->errorString();
        return;
...
void FanfouMicroBlog::slotCreateFriendship( KJob* job )
{
    if ( job->error() ) {
        qWarning() << "Job Error: " << job->errorString();
        return;
...
void FanfouMicroBlog::slotRemoveFriendship( KJob* job )
{
    if ( job->error() ) {
        qWarning() << "Job Error: " << job->errorString();
        return;
...
QDateTime FanfouMicroBlog::dateFromString( const QString &date )
{
    char s[10];
...
void FanfouMicroBlog::readPostFromJsonMap( const QVariantMap& varmap, Choqok::Post* post )
{
...
void FanfouMicroBlog::readDMessageFromJsonMap( Choqok::Account* account, const QVariantMap& varmap, Choqok::Post* post )
{
    post->isPrivate = true;
...
void FanfouMicroBlog::readUserFromJsonMap( const QVariantMap& varmap, Choqok::User* user )
{
